    ...
    <section className="p-6 bg-gradient-to-b from-white via-slate-50 to-gray-50 min-h-screen">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-3" ref={timelineRef}>
          {/* Timeline column */}
          <div className={`col-span-1 lg:block ${isMenuOpen ? "block" : "hidden"}`}>
                ...
                <div className="mt-4 space-y-2">
                  {data.generations.map((g, i) => (
                    <motion.button
                      ...
                      animate={{
                        backgroundColor:
                          g.id === activeId ? g.color : "rgba(243,244,246,0)",
                        boxShadow:
                          g.id === activeId ? `0 4px 14px 0 ${g.color}55` : "none",
                      }}
                      whileHover={{
                        backgroundColor:
                          g.id !== activeId ? "#f3f4f6" : g.color,
                        scale: 1.03,
                      }}
                      whileTap={{ scale: 0.98 }}
                    >
          ...
          <div className="lg:hidden flex justify-end mb-4">
            <button
              ...
              className="p-2 rounded-md bg-gray-200 text-gray-600 hover:bg-gray-300"
            >
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mt-10 p-4 bg-gray-100 rounded-lg shadow-md gap-4 md:gap-0">
          ...
          <button
            onClick={() => {
              const currentIndex = data.generations.findIndex(
                (g) => g.id === activeId
              );
              if (currentIndex === 0) {
