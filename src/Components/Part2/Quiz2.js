...
export default function App() {
    ...
    const handleAnswerClick = (index) => {
        ...
        const isCorrect = quizQuestions[currentQuestionIndex].answerOptions[index].isCorrect;
    ...
    const handleNextQuestion = () => {
        if (!isAnswerSubmitted) {
            setNavigationWarning(`Please answer the current question before moving on.`);
            return;
        }
        setNavigationWarning('');
        setShowHint(false);
    ...
    const handlePreviousQuestion = () => {
        if (!isAnswerSubmitted) {
            setNavigationWarning(`Please answer the current question before moving back.`);
            return;
        }
        setNavigationWarning('');
        setShowHint(false);
    ...
    const handlePlayAgain = () => {
        // Reset all states and re-shuffle the questions
        setQuizQuestions(shuffleArray(originalQuizData));
    ...
    const getButtonClass = (index) => {
        ...
        if (quizQuestions[currentQuestionIndex].answerOptions[index].isCorrect) {
    ...
    const getIcon = (index) => {
        ...
        if (quizQuestions[currentQuestionIndex].answerOptions[index].isCorrect) {
            return <CheckCircle2 className="w-5 h-5 ml-2" />;
        } else if (index === selectedAnswerIndex) {
    ...
    if (showResults) {
        return (
            <div className="flex flex-col items-center justify-center h-screen bg-yellow-100 p-4 font-sans">
                ...
                >
                    <Link
                        to="/parts/prt2"
                        className="inline-flex items-center px-6 py-2 bg-blue-500 
      ...
      transition-all duration-300 ease-out"
                ...
                <div className="w-full max-w-xl p-8 bg-white rounded-3xl shadow-2xl border-4 border-yellow-500 text-center transform transition-all duration-500 scale-100 hover:scale-105">
                    ...
                    <button
                        ...
                        className="w-full sm:w-auto px-8 py-3 bg-yellow-500 text-white font-bold text-lg rounded-full shadow-lg hover:bg-yellow-600 transition-transform transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-yellow-300 flex items-center justify-center mx-auto"
                    >
